         ...
         if(fd < 0) {
            ...
            if(encoding_opus) {
               ...
               if(offset < 0) {
                  ...
                  XLOGD_ERROR("unsupported wave file format - channel qty <%u> sample rate <%u> sample size <%u> encoding <%d>", format.channel_qty, format.sample_rate, format.sample_size, format.encoding.type);
                  stream_begin_failure = true;
